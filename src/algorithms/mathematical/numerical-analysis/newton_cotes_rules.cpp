...
enum class Rule {
    Trapezoidal = 0, SimpsonFirst = 1, SimpsonSecond = 2, Boole = 3
};

double NewtonCotesRules(const Rule& rule, std::function<double(const double&)> Function, const double& a, const double& b, const unsigned& n) {
...
int main() {
    ...
    assert(numSplits % 2 == 0);
    auto Function = [](const double& x) -> double {
        return std::cosh(x);
    };
    std::cout << NewtonCotesRules(static_cast<Rule>(rule), Function, start, end, numSplits) << std::endl;
