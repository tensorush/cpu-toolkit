...
enum class Difference {
    Forward = 0, Central = 1, Backward = 2
};
...
int main() {
    ...
    auto component_1 = [](const std::vector<double>& point) -> double {
        return std::pow(point[0] + point[1], 2);
    };
    auto component_2 = [](const std::vector<double>& point) -> double {
        return std::cosh(point[1] - point[0]);
    };
